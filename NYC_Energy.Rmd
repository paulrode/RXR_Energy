...
alldata %>% mutate(Month = month(`Start Date`), Period = (alldata$`End Date`- alldata$`Start Date`)) %>% mutate(i = interval(ymd(`Start Date`), ymd(`End Date`)), Usage_Day = Usage /as.numeric(Period)) -> alldata
UsageData <- select(alldata, Building,  'Meter Type', 'Meter Number',`Start Date`,`End Date`, i, Usage_Day)


# Summeraze data so there is one value per daterange
UsageData %>% group_by(Building, `Meter Type`, i, `Start Date`, `End Date`) %>% summarise(`Usage/day` = mean(Usage_Day)) -> lookuptable
head(lookuptable)
glimpse(alldata)
```
