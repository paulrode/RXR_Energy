...
EnergyTypes <- unique(alldata$`Meter Type`)
...
k <- 15
UtilityDataNoCovid %>% filter(month(date) > 4 & month(date) < 11 & building == buildings[k]) %>% ggplot( aes(CDD, varriable, color = "green")) +
    geom_point() +
    geom_smooth(method=lm , color="red", se=TRUE) +
    facet_grid(building~.)
