...
alldata_perday %>% mutate(Elect61[Elect61BWay > Elect61$`Start Date` & Elect61BWay < Elect61$`End Date`,6])
...
filter(Elect61, date > `Start Date` & date < `End Date`) %>% select(Usage_Day) -> value[i,2]

}
value <- as.data.frame(x = 0, y = 0)
