...
cdd %>% select(1:5) %>% gather(key = "key", value = "CDD", -1) -> cdd1
hdd %>% select(1:5) %>% gather(key = "key", value = "DHH", -1) -> hdd1
...
dailyElect %>% group_by(year(dates), month(dates)) %>% summarise(kWh = sum(UsagePerDay)) -> monthlyElect
