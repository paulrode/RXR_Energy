...
k <- 1
#Heating
UtilityDataNoCovid %>% filter(building == buildings[k] & type == "Steam") %>% filter(month(date) < 4 | month(date) > 10  ) %>% ggplot( aes(HDD, varriable, color = "blue")) +    geom_point() +
    geom_smooth(method=lm , color="red", se=TRUE) 

#Cooling 
UtilityDataNoCovid %>% filter(building == buildings[k] & type == "Steam") %>% filter(month(date) > 4 & month(date) < 10  ) %>% ggplot( aes(HDD, varriable, color = "black")) +    geom_point() +
