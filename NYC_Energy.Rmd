...
# Establish end points on a daily dataframe.
...
as.data.frame(Elect61) -> Elect61


# Create a df with all days of the year for normarilizing useage per month. 
Elect61BWay <- seq(as.Date(Elect61$`Start Date`[1]), as.Date(Elect61$`End Date`[length(Elect61$`End Date`)]), by = "day")
alldata_perday <- as.data.frame(Elect61BWay)

# Using filters and brackets look up kWh/day for each day of the year 
upd <- data.frame(UsagePerDay = seq(1:length(alldata_perday$Elect61BWay)))
for(i in 1:length(alldata_perday$Elect61BWay)) {
  date <- alldata_perday$Elect61BWay[i]
  as.numeric(filter(Elect61, date > `Start Date` & date <= `End Date`) %>% 
  select(Usage_Day)) %>% as.data.frame()-> upd[i,1]
  upd[1,1] <- Elect61[1,6]
}

alldata_perday <- cbind(alldata_perday, upd)
head(alldata_perday)
qplot(alldata_perday$Elect61BWay, alldata_perday$UsagePerDay) 
