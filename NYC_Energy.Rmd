...
alldata %>% mutate(Month = month(`Start Date`), Period = (alldata$`End Date`- alldata$`Start Date`)) %>% mutate(i = interval(ymd(`Start Date`), ymd(`End Date`)), kWh_Day = Usage /as.numeric(Period)) -> alldata
UsageData <- select(alldata, 'Meter Type', 'Meter Number', 'i', kWh_Day)




# Summeraze data so there is one value per daterange
alldata %>% group_by(i) %>% summarise(`kWh/day` = mean(kWh_Day)) -> lookuptable
head(lookuptable)
...
Types <- unique(alldata$`Meter Type`)
...
# alldata_perday %>% mutate("kWh/day" == 
                            
alldata[alldata$`Start Date` < as.Date("2018-03-23"),]
alldata[alldata$`Start Date` == alldata_perday$Dates,]
alldata_perday %>% mutate("kWh_day" == alldata$kWh_Day[(alldata_perday$Dates > alldata$`Start Date` & alldata_perday$Dates < alldata$`End Date`),])

alldata$kWh_Day[alldata_perday$Dates > alldata$`Start Date` & alldata_perday$Dates < alldata$`End Date`]

ad <- mdy("4/3/2018")
alldata %>% filter(ad%within%i) %>% select(i, kWh_Day) %>% head()
