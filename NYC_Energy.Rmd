...
alldata %>% group_by(Building, `Meter Type`, `End Date`) %>% summarise(Usage = sum(Usage), Usage_Day=sum(Usage_Day)) -> alldata
...
#for(j in 6:length(alldata$Building)) {

j <- 5

alldata %>% filter(Building == buildings[j] & `Meter Type` == "Electric") -> Elect

data.frame(dates = seq(as.Date(Elect$`End Date`[1]), as.Date(Elect$`End Date`[length(Elect$`End Date`)]), by = "day"))  -> alldata_perday
...
monthlyElect %>% mutate("month" = month(date, label = TRUE, abbr = FALSE)) %>%
  mutate("building" = buildings[j]) -> monthlyElect
colnames(monthlyElect) = c("year", "month1", "kWh", "date", "month", "building")
monthlyElect <- as.data.frame(monthlyElect)

monthlyElect %>% mutate(datekey = paste(`month`,`year`)) -> monthlyElect

monthlyElect %>% select(building, datekey, kWh, date) -> monthlyElect
...
monthlyElect[-1,] -> monthlyElect
  monthlyElect %>% mutate(varriable = kWh - min(monthlyElect$kWh)) -> monthlyElect
...
```{r, echo=FALSE}
monthlyELectNoCovid <- monthlyELectNoCovid[-1,]

plot1 <- ggplot(monthlyELectNoCovid, aes(date, varriable)) +
  geom_col() 

plot2 <- ggplot(monthlyELectNoCovid, aes(date, CDD)) +  
    geom_line()

plot3 <- ggplot(monthlyELectNoCovid, aes(date, HDD)) +
  geom_line()

grid.arrange(plot1, plot2, plot3, nrow= 3, ncol=1)
  
plot(monthlyELectNoCovid$CDD, monthlyELectNoCovid$varriable)
plot(monthlyELectNoCovid$HDD, monthlyELectNoCovid$varriable)
plot(monthlyELectNoCovid$HDD+monthlyELectNoCovid$CDD, monthlyELectNoCovid$varriable)


lm(kWh ~ CDD, monthlyELectNoCovid, subset = (month(date)>5 & month(date) <10))
