...
UtilityDataNoCovid %>%filter(building == building[k] & type == "Electric") %>% select(useage) %>% ts(start = c(2017, 2), frequency = 12) -> UtilityTS
...
auto.arima(UtilityTS) ->ts
autoplot(UtilityTS)
residuals(ts)

# Investigate transformations
# take first differene to remove trend
DUT <- diff(UtilityTS)
plot.ts(DUT)

#looking for seasonality as trend looks stationary
...
UtilityData %>% filter(building == "237 Park" & type == "Electric") %>% mutate("DD" = CDD - HDD) %>% select(date, useage, DD) %>% as.tibble() -> elec
