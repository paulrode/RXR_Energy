...
#Remove NA's
ies_elect_data <- na.omit(ies_elect_data)

# Need to see if data time feather is in dttm format, if not use the linte below. 
glimpse(ies_elect_data)
# mdy_hm(ies_elect_data$`Date`) -> ies_elect_data$`Date`

#Clean up feather names to make reporting readable 
...
glimpse(ies_elect_data)
glimpse(na.omit(ies_elect_data))
...
ies_elect_data_week %>% ggplot(aes(`week`, na.rm=TRUE)) +
  geom_line(aes(y=`baseload`, colour = "red", na.rm=TRUE)) +
  geom_line(aes(y=`total`, color = "blue"))

ies_elect_data %>% ggplot(aes(x=`Date`, y = `Electric`, na,rm = TRUE))+
...
ies_elect_data_day %>% ggplot(aes(x = week, y =  `Electric`)) +
  geom_boxplot(aes(group =week))

ies_elect_data_day %>% mutate(month = month(`Date`)) %>% ggplot(aes(x = month, y =  `Electric`)) +
...
```{r, echo=FALSE}
#make a timeseries 
ies_data_ts <- ts(ies_elect_data, start = c(2019,4), frequency = 365)

 ies_data_ts <- ts(ies_elect_data, start = c(2019, as.numeric(format(ies_elect_data$Date, "%j"))), frequency = 365)                                                    

 glimpse(ies_data_ts)
 glimpse(ies_elect_data)
                                                     as.numeric(format(ies_data_business_hours$`Date/Time`, "%j")), frequency = 365)



#Looking at relationships
GGally::ggpairs(ies_elect_data)
